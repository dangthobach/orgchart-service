...
/**
 * Standalone performance test without Spring context - Pure Java performance testing
 * Tests the complete pipeline: Mock Data Generation → Excel Writing → Excel Reading
 */
public class StandaloneUserExcelPerformanceTest {
    
    private static final Logger logger = LoggerFactory.getLogger(StandaloneUserExcelPerformanceTest.class);
    
    private static final int TOTAL_RECORDS = 500_000;
    private static final String EXCEL_FILE_PATH = "target/users_500k_standalone_test.xlsx";
    ...
    public void testStep1_GenerateAndWriteToExcel() throws Exception {
        logger.info("================================================================================");
        logger.info("🚀 STEP 1: GENERATE MOCK DATA AND WRITE TO EXCEL (STANDALONE)");
        logger.info("================================================================================");
        ...
        try {
            ...
            java.io.File outputFile = new java.io.File(EXCEL_FILE_PATH);
            if (outputFile.exists()) {
                long fileSize = outputFile.length();
                logger.info("✅ File verification:");
                logger.info("   File size: {} MB ({} bytes)", fileSize / (1024 * 1024), fileSize);
                logger.info("   Average bytes per record: {}", fileSize / TOTAL_RECORDS);
            }
    ...
    public void testStep2_ReadExcelAndProcess() throws Exception {
        logger.info("================================================================================");
        logger.info("📖 STEP 2: READ EXCEL FILE WITH STREAMING PROCESSING (STANDALONE)");
        logger.info("================================================================================");
        ...
        try {
            ...
            // Batch processor with validation and simulated processing
                ...
                for (User user : batch) {
                    try {
                        ...
                        if (validateUser(user)) {
                            ...
                            String processed = user.getFirstName() + " " + user.getLastName();
                        } else {
    ...
    /**
     * COMBINED TEST: Complete pipeline from generation to processing
     */
    @Test
    public void testCompletePipeline() throws Exception {
        logger.info("================================================================================");
        logger.info("🔄 COMPLETE PIPELINE TEST: GENERATION → EXCEL → PROCESSING (STANDALONE)");
        logger.info("================================================================================");
        ...
        try {
            ...
            logger.info("✅ Pipeline: Mock Data → Excel Write → Excel Read → Processing");
            logger.info("✅ All optimizations applied and verified successfully");
            
            // File size analysis
            java.io.File outputFile = new java.io.File(EXCEL_FILE_PATH);
            if (outputFile.exists()) {
                long fileSize = outputFile.length();
                logger.info("📊 Final file analysis:");
                logger.info("   File size: {} MB", fileSize / (1024 * 1024));
                logger.info("   Compression ratio: {:.2f}x", (TOTAL_RECORDS * 200.0) / fileSize); // Assume ~200 bytes per record uncompressed
            }
    ...
    private void analyzeStep2Performance(long readDuration, long processedRecords, long validRecords, long errorRecords) {
        ...
        if (readRate < 10000) {
            ...
            logger.info("   - Optimize processing logic");
