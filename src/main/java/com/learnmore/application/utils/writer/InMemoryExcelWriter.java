...
/**
 * In-memory Excel writer for small datasets with full formatting support
 * Best for < 10K records with complex formatting requirements
 */
@Slf4j
public class InMemoryExcelWriter<T> implements ExcelWriter<T> {
    ...
    public InMemoryExcelWriter(Class<T> beanClass, ExcelConfig config) throws ExcelProcessException {
        if (beanClass == null) {
            throw new ExcelProcessException("Bean class cannot be null");
        }
        if (config == null) {
            throw new ExcelProcessException("Excel config cannot be null");
        }
        ...
        log.info("InMemoryExcelWriter initialized for {} with {} fields", 
    ...
    private void writeDataRows(Sheet sheet, List<T> data, Workbook workbook) throws ExcelProcessException {
        Map<Class<?>, CellStyle> styleCache = new HashMap<>();
        
        for (int i = 0; i < data.size(); i++) {
            T record = data.get(i);
            Row row = sheet.createRow(i + 1); // +1 for header row
            
            writeRecord(row, record, workbook, styleCache);
    ...
    private void writeRecord(Row row, T record, Workbook workbook, Map<Class<?>, CellStyle> styleCache) 
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            try {
                ...
                setCellValue(cell, value, field.getType(), workbook, styleCache);
    ...
    private void setCellValue(Cell cell, Object value, Class<?> fieldType, Workbook workbook, 
                             Map<Class<?>, CellStyle> styleCache) {
        if (value == null) {
            cell.setCellValue("");
            return;
        }
        ...
        if (fieldType == String.class) {
            ...
            applyCellStyle(cell, fieldType, workbook, styleCache);
    ...
    private void applyCellStyle(Cell cell, Class<?> fieldType, Workbook workbook, 
                               Map<Class<?>, CellStyle> styleCache) {
        CellStyle style = styleCache.get(fieldType);
        
        if (style == null) {
            style = createDataStyle(workbook, fieldType);
            styleCache.put(fieldType, style);
        }
        
        cell.setCellStyle(style);
    }
    
    private CellStyle createDataStyle(Workbook workbook, Class<?> fieldType) {
        CellStyle style = workbook.createCellStyle();
