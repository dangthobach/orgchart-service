...
/**
 * In-memory processor for small Excel files
 */
@Slf4j
public class InMemoryProcessor<T> implements ExcelProcessor<T> {
    
    private final Class<T> beanClass;
    private final ExcelConfig config;
    private final ProcessingStatistics statistics;
    
    public InMemoryProcessor(Class<T> beanClass, ExcelConfig config) {
        this.beanClass = beanClass;
        this.config = config;
        this.statistics = new ProcessingStatistics();
        this.statistics.setStrategy("IN_MEMORY");
    }
    ...
    public void processStream(InputStream inputStream, Consumer<List<T>> consumer) 
        ...
        try {
            ...
            // In a real implementation, this would use POI's in-memory processing
