...
/**
 * XSSF processor for medium Excel files
 */
@Slf4j
public class XSSFProcessor<T> implements ExcelProcessor<T> {
    
    private final Class<T> beanClass;
    private final ExcelConfig config;
    private final ProcessingStatistics statistics;
    
    public XSSFProcessor(Class<T> beanClass, ExcelConfig config) {
        this.beanClass = beanClass;
        this.config = config;
        this.statistics = new ProcessingStatistics();
        this.statistics.setStrategy("XSSF");
    }
    ...
    public void processStream(InputStream inputStream, Consumer<List<T>> consumer) 
        ...
        try {
            ...
            // In a real implementation, this would use POI's XSSF processing
